...
/** 把草稿(增/删/改事件与边)应用到故事图，得到"生效后"的图。端点不存在的边自动剔除。 */
export function applyDraft(story: StoryGraph, d: Draft): StoryGraph {
  if (!d.addEvents.length && !d.addEdges.length && !d.removeEventIds.length && !d.removeEdges.length && !d.updateEvents.length) return story;
  const rmEv = new Set(d.removeEventIds);
  const patch = new Map(d.updateEvents.map((u) => [u.id, u.patch]));
  const events = story.events
    .filter((e) => !rmEv.has(e.id))
    .concat(d.addEvents.filter((e) => !rmEv.has(e.id)))
